New-PSDetourHook -DllName mskeyprotect.dll -MethodName SIDKeyProtKeyUnprotect {
    ...
    $this.State.WriteLine(
        'SIDKeyProtKeyUnprotect(Descriptor: 0x{0:X8}, 0x{1:X8}, MemAllocFunc: 0x{2:X8}, Window: 0x{3:X8}, Data: 0x{4:X8}, DataLength: 0x{5:X8}, Flags: 0x{6:X8})',
    ...
    $this.State.WriteLine('SIDKeyProtKeyUnprotect -> Result: 0x{0:X8} CEK: {1}', $res, [Convert]::ToHexString($returnData))
    $this.State.WriteLine()
